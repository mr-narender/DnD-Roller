...
-   [W3C Markup Validator](https://validator.w3.org/#validate_by_input) - [Results](https://github.com/JuanBrachoDev/DnD-Roller/tree/master/docs/validation/html)
    >   Results achieved after fixing accessibility issues, specificically missing **aria-label**(s) in input fields accross all pages. Ignoring warnings in index.html as adding headers wouldn't fit the design of the page.
...
-   [Google Chrome Lighthouse](https://developers.google.com/web/tools/lighthouse) - [Results](https://github.com/JuanBrachoDev/DnD-Roller/tree/master/docs/validation/lighthouse)
    >   Initially the tests showed issues with performance while advising to pre-load the style file with fonts in it. So added preload links to common.css in all three pages and results where satisfactory.
...
-   #### As a player I need to:
    ...
    - Be able to store my character's stats so I can easily return to it between sessions.
        > I can **input** my character's most important stats into the Character Sheet page to calculate important modifiers, and they **stay in place** even if I reopen the page later on.
...
| Feature | Expected Behavior | Testing Method | Result |
| :-: | :-: | :-: | :-: |
...
-   Friends and family members were asked to review the site and documentation to point out any erros, bugs and/or user         experience issues.
...
-   Character Sheet labels showed NaN on first run of the page.
       > Bug occured after initializing the character sheet fields with their **Local Storage** values. Added functions that checked if any value had been stored in **Local Storage** before fetching said value, and left the field blank if none was found.
